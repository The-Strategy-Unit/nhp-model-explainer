...
There are many elements that can change how much people need hospitals in the future. The main four main components that the model considers are demographic growth, non-demographic growth, unmet demand and mitigation.

```{mermaid}
flowchart TD
    B(Demographic <br> growth ⬆️) --> A(NHP demand model)
    c(Non-demographic <br> growth ⬆️) --> A(NHP demand model)
    D(Unmet <br>demand ⬆️) --> A(NHP demand model)
    E(Mitigation ⬇️) --> A(NHP demand model)
```
...
The model uses real hospital data as a baseline, but hospitals don’t always fully meet the nation’s needs. Some patients face long wait times, travel long distances for care, or opt for private healthcare. We adjust for this unmet demand in the model to ensure we can better address future needs.
...
- Shifting care out of hospitals to community settings
- Preventing health issues early with public health measures
- Improving efficiency, through better healthcare and less invasive procedures 
